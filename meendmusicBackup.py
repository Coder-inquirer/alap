...
def dampen(sine_arr_x, half_length=half_length):
    #global half_length
    global prev_ampl
    #sine_arr_x = [a*b for a,b in zip(sine_arr_x,list(map(damper,range(len(sine_arr_x)))))]
...
def write_to_wav(fbeg=freq, fend=freq, wavfile=wav_file, duration=duration, frate=frate, method='linear', pluck=True):
    ...
    for i in range(len(harmonics)):
        ...
        if pluck==False and dampening==True:
            ...
            prev_ampl = 1
        if dampening==True:
            dampen(ch,half_length/(i+1)**half_length_factor)
            prev_ampl*=math.exp(-len(sine_arr_x)/half_length)
        sine_arr_x += harmonics[i]*ch
    prev_ampl = temp1_prev_ampl
    ...
    print(sine_arr_x[0],sine_arr_x[1],sine_arr_x[-2],sine_arr_x[-1])
    for s in sine_arr_x:
